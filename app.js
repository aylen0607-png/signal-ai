...
function renderFilters() {
  const companies = [...new Set(videos.map((video) => video.company))].sort((a, b) => a.localeCompare(b));
  filterRow.innerHTML = `<button class="chip ${selectedCompany === 'all' ? 'selected' : ''}" data-company="all" type="button">全部 <span>${videos.length}</span></button>` + companies.map((company) => {
    const [, accent] = colorFor(company);
    const total = videos.filter((video) => video.company === company).length;
    return `<button class="chip ${selectedCompany === company ? 'selected' : ''}" data-company="${company}" type="button"><i class="company-dot" style="background:${accent}"></i>${company} <span>${total}</span></button>`;
  }).join('');
}
...
filterRow.addEventListener('click', (event) => { const button = event.target.closest('.chip'); if (!button) return; selectedCompany = button.dataset.company; renderFilters(); render(selectedCompany); });
