...
function colorFor(company) { return { OpenAI: ['#283b38', '#d2ff65'], 'Google DeepMind': ['#1c2943', '#7fa6ff'], Gemini: ['#263a6a', '#8aa9ff'], Anthropic: ['#473c34', '#e6a879'], 'Meta AI': ['#18336a', '#76a7ff'], 'Mistral AI': ['#5d281f', '#ff8a55'] }[company] || ['#303740', '#aeb9c0']; }
...
function renderFilters() {
  const preferredCompanies = ['OpenAI', 'Google DeepMind', 'Claude（Anthropic）', 'Gemini'];
...
function openPlayer(button) {
  ...
  modal.innerHTML = `<section role="dialog" aria-modal="true" aria-label="播放 ${escapeHtml(button.dataset.videoTitle)}" style="width:min(900px,100%);position:relative"><button type="button" aria-label="关闭视频" style="position:absolute;right:0;top:-42px;border:0;background:transparent;color:white;font-size:28px;cursor:pointer">×</button><div style="position:relative;padding-top:56.25%;background:#000"><iframe src="https://www.youtube.com/embed/${escapeHtml(button.dataset.videoId)}?autoplay=1&rel=0" title="${escapeHtml(button.dataset.videoTitle)}" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen style="position:absolute;inset:0;width:100%;height:100%;border:0"></iframe></div></section>`;
